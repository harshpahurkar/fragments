...
module.exports = async (req, res) => {
  ...
  const tagFilter = req.query && req.query.tag;  let fragments = await listFragments(owner, expand);
  ...
  if (tagFilter && expand) {
    ...
    fragments = fragments.filter((f) => f.tags && f.tags.some((tag) => tagsToMatch.includes(tag)));
  }
